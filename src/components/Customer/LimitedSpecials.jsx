        ...
        {items.map((it) => (
          <div key={`special-${it.id}`} className="flex-shrink-0 w-32 text-center relative">
            <div className="h-20 w-20 bg-white rounded-full mx-auto mb-1 overflow-hidden relative">
              <img src={it.imagePath} alt={it.name} className="w-full h-full object-cover" />
              {it.discount > 0 && (
                <span className="absolute -top-1 -right-1 bg-green-600 text-white text-sm px-1 rounded">
                  Save ${it.discount}
                </span>
              )}
            </div>
